...
import jsonpickle
from cfgm_common.exceptions import (DatabaseUnavailableError, VncError)
...
class DMEtcdDB(VncObjectEtcdClient, DeviceManagerDBMixin):
    ...
    def _fetch_pnf_resources(self, si_id):
    ...
    def add(self, path, key, value):
        ...
        try:
            ...
            return True
        except (DatabaseUnavailableError, VncError):
            return False
    # end add
    ...
    def get_pnf_resources(self, vmi_obj, pr_id):
        ...
        self._object_db.put_kv(self._pnf_resources_path(si_id),
    ...
    def delete_pnf_resources(self, si_id):
        ...
        """
        # NOTE used in self.handle_pnf_resource_deletes
        # and ServiceInstanceDM.delete
        pnf_resources = self.pnf_resources_map.get(si_id, None)
        if not pnf_resources:
            pnf_resources = self._fetch_pnf_resources(si_id)
        if not pnf_resources:
            return
        ...
        # deallocate unit_id and unset it in specific physical interface's config
    ...
    def handle_pnf_resource_deletes(self, si_id_list):
        ...
        # NOTE used in DeviceManager.__init__
    ...
    def get_db_info(cls):
        db_info = [
            (cls._ETCD_DEVICE_MANAGER_PREFIX, [cls._ETCD_PR_VN_IP_TABLE_KEY]),
        ]
        return db_info
    # end get_db_info
